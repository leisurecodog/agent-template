...
# 教訓分兩層：repo 相關 → <repo_root>/tasks/lessons.md；使用者個人 → 固定
...
# repo 教訓 → <repo_root>/tasks/lessons.md（依 git root 定位）
repo_root=""
...
# 寫入目標與格式的規範來源：~/.claude/commands/lessons-update.md（單一基準）。
# 本 prompt 只注入動態路徑（$tasks_dir / $mem_dir）取代命令內屬於本互動的具體位置，
# 其餘目標與格式規則照 lessons-update.md 執行。
#
...
作業：依【lessons-update 規範】判斷這次互動裡有沒有值得寫進長期記憶的可重用教訓。你**沒有
...
WRITE_REPO)
	if [ -n "$tasks_dir" ]; then
		...
		if [ -n "$content" ]; then
			printf '%s\n' "$content" >"$tasks_dir/lessons.md"
			printf '%s\n' "[$(date '+%F %T')] repo: $tasks_dir/lessons.md" >>"$HOME/.claude/scripts/.self-learn-log"
		fi
	fi
	;;
WRITE_PERSONAL)
